...
Now I've got a docs site that comfortably supports multiple versions, but there's one thing left to do. I noticed that links between pages within a remote version were going to the wrong places. Normally, I would write my links as absolute paths, i.e., `md›[click here](/getting-started)`. However, if I used that path within a page living at `/v1`, it wouldn't take the user to `/v1/getting-started` as I would have hoped. Instead, they would go to `/getting-started` at the site root. Understandable. 🤷
...
There are a few things going on in this component:
...
To take advantage of this custom link component in my MDX pages, I first import the `PathContext` from the link component file and pass the template's `path` prop as the `value` of `PathContext.Provider`. Then I wrap the `MDXRenderer` in an `MDXProvider` and a mapping of HTML elements to the React component that I'd like to substitute them with. In this case, I want to subsitute `a` tags with the `RelativeLink` component defined above.
  ...
  return (
    <>
      ...
      <PathContext.Provider value={path}>
        <MDXProvider
          components={{
            // substitute anchors with custom link component
            a: RelativeLink
          }}
        >
          <MDXRenderer>{body}</MDXRenderer>
...
// page query same as above
```

## Complete example

And that's versioned documentation with Gatsby in a nutshell! 🥥 I hope you found this guide informative, and maybe even inspired to support multiple versions in your own docs.
