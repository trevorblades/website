...
- using gatsby link for internal links
- being able to link between pages with an older version without needing to consider the version path -> relative links
- mdx provider + custom link component
- make page `path` prop available to all links using context
