...
With this information, we set the `path` of each page to include the branch name in the URL. Current version docs will begin with `/` while pages for other versions will begin with `/branch-name/`, or `/v1/` in this example.
...
This method works, however the `node.slug` property for pages sourced from git include the full directory structure in them, e.g., `docs/content/getting-started`. For this to work properly, this content directory part must be stripped out.
...
Using this regular expression, we can do `js›node.slug.replace(regex, '$1')` to strip out the content directory from the slug.

```js
makeRe(glob, {capture: true}); // regex with capture group for wildcard parts
```
