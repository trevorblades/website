...
I drew a big spiral in my notebook and counted the insets and outsets for each side. I went about 10 sides deep to see if any pattern would reveal itself. Here's what that data looks like:
...
$$
I = \lfloor\frac{n}{S}\rfloor + \lfloor\frac{n - 2}{S}\rfloor
$$

In JavaScript, we could apply this knowledge to write a function that returns the side length for any side in the spiral, given the current `side`, the total `numSides`, and the `originalLength` of the first segment.

```js
function getSideLength(side, numSides, originalLength) {
  const outsets = 2 * Math.floor((side - 1) / numSides);
  const insets =
    Math.floor(side / numSides) + Math.floor((side - 2) / numSides);
  return originalLength - insets - outsets;
}
```
