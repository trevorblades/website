...
The outsets column presents a clear pattern. Every three sides, the number of outsets increments by two. In mathematical notation, we could express this formula where $O$ is the number of outsets, $S$ is the current side, and $N$ is the total number of sides on the shape—3 in the case of a triangle.

$$
O = 2\lfloor\frac{S - 1}{N}\rfloor
$$
...
$$
I = \lfloor\frac{S}{N}\rfloor + \lfloor\frac{\max(S - 2, 0)}{N}\rfloor
$$
...
Calculating these radii is a complex task. **If the number of sides is odd**, the formula for the circumradius is as follows, where the variables are the same as above.
...
R_i = \left\{
  \begin{array}{ll}
    w / 2 & S / 2\text{ is even} \\
    w / 2 \times \cos\frac{\theta}{2} & N / 2\text{ is odd}
  \end{array}
\right.
$$
...
For squares and triangles, the original side length is equal to the width of the shape—easy! For shapes with more than four sides, I need to calculate that too. For shapes with an **odd number of sides**, this can be calculated using the circumradius $R_c$ and the exterior angle $\theta$.
...
Now that I have the height of the shape, I like to center it within a square bounding box. I create a `div` with equal width and height, as provided by the component's `width` prop. Then I find the difference between the box size and the shape height as calculated above. Divide that number by two, and that's the amount I need to offset my shape to vertically center it.

To horizontally center the shape, I need to move the first side into the middle of the box. I can do that by calculating the difference between the box size and side length, and then dividing by two as with the vertical padding.
...
<div
  style={{
    width,
    height: width,
    paddingTop: (width - height) / 2,
    paddingLeft: (width - sideLength) / 2
  }}
>
...
The last change I need to make for this spiral to be "multi-polygonal" has to do with the calculation of inner side lengths, and the outsets in particular. When I was rendering a triangle, I subtracted outsets from the original side length to end up with each inner side length. For shapes with more than four sides, I must instead _add_ outsets to perform the calculation correctly.

Simple enough—I can perform a check after calculating my outset and invert the number if we're dealing with a shape with more than four sides.
